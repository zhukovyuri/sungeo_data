...
# nohup R -e 'source("/media/zhukov/dropbox2022/Dropbox/SUNGEO/Code/Population/Geoprocessing/ghspop_geoprocess.R")'  > sungeo_ghspop.log 2>&1 &  
    ...
    if(!skip_existing|(skip_existing&!file.exists(sub_dirz[k0,fnn]))){
			...
			if(sum(sf::st_is_empty(map))==0&nrow(map)>0){
				...
				pop.mat <- parallel::mclapply(1:nrow(ticker),function(t){
					...
					# plot(f_tif_m0)
					# dev.off()
					# rnkz <- map_z[,rank(pop_km2)/max(rank(pop_km2))]
