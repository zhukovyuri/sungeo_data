...
# Data source
datsrz <- "BuiltUp/GHSBS"
dir("Data/BuiltUp/GHSBS/Raw")
	    ...
	    if(!skip_existing|(skip_existing&!file.exists(sub_dirz[k0,fnn]))){
			...
			if(sum(sf::st_is_empty(map))==0&nrow(map)>0){
				...
				bu.mat <- parallel::mclapply(1:nrow(ticker),function(t){
					...
					# print(t)
					if(extra_verbose){if(grepl("0$|5$",as.character(ticker[t,YEAR]))){print(sub_dirz[k0,paste(geoset,adm,iso3,adm_yr,as.character(ticker[t,YEAR]))])}
					}
					...
					# edgar_nc <- terra::rast(paste0(filez_t[,fn]))
					...
					# Zonal stats
					map_z <- terra::zonal(x=f_tif_m0,z=map00.0 %>% sf::st_as_sf() %>% terra::vect(),fun="mean",na.rm=TRUE) %>% data.table::setnames("builtsurf_mean")
					map_z_sum <- terra::zonal(x=f_tif_m0,z=map00.0 %>% sf::st_as_sf() %>% terra::vect(),fun="sum",na.rm=TRUE) %>% data.table::setnames("builtsurf_sum")
					map_z_ct <- terra::extract(f_tif_m0, map00.0 %>% sf::st_as_sf() %>% terra::vect()) %>% group_by(ID) %>% dplyr::summarise(builtsurf_ct=n()) 
					
					# Save
					map_z <- map_z %>% data.table::as.data.table() %>% .[,POLYGON_ID:=poly_idz] %>% .[,YEAR:=ticker[t,YEAR]] %>% .[,builtsurf_sum:=map_z_sum$builtsurf_sum] %>% .[,builtsurf_ct:=map_z_ct$builtsurf_ct] %>% dplyr::select("POLYGON_ID","YEAR",dplyr::everything())
					

					# # # Preview
					# plot(f_tif_m0)
					# dev.off()
					# rnkz <- map_z[,rank(builtsurf_sum)/max(rank(builtsurf_sum))]
