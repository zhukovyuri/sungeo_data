...
# Fix character encoding issue (convert all to ASCII)
geo_mat <- geo_mat %>% dplyr::mutate_if(sapply(.,is.character),function(.){stringi::stri_encode(., "", "UTF-8") %>% stringi::stri_trans_general(., "Latin-ASCII")}) %>% dplyr::mutate(ISO3 = countrycode(ctr_n,"country.name","iso3c")) %>% as.data.table()
...
yr_list <- mclapply(1:length(yrz),function(y0){
  ...
  save_list <- lapply(seq_along(clea_cntz),function(k0){
    ...
    save(clea_mat,file=paste0("Elections/CLEA/Preprocessed/CLEA_",clea_cntz[k0],"_",yrz[y0],".RData"))
  })
