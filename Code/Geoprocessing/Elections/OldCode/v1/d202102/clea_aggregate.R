...
sub_dirz <- dir("Elections/CLEA/Preprocessed") %>% str_split("_|.RDS",simplify=T) %>% .[,2:3] %>% as.data.frame(stringsAsFactors=T) %>% data.table::setnames(c("iso3","yr")) %>% dplyr::mutate(iso3 = iso3 %>% as.character(), yr = yr %>% as.character() %>% as.numeric(), f0 = list.files("Elections/CLEA/Preprocessed",full.names=T)) %>% arrange(iso3,yr) %>% as.data.table()
which(sub_dirz$iso3=="USA"&sub_dirz$yr==2018)
    ...
    if(nrow(admz_dir_)>0){
      ...
      for(a0_ in 1:nrow(admz_dir_)){
        ...
        if(!skip_existing|(skip_existing
          ...
          # Load CLEA
          clea_mat <- readRDS(sub_dirz[k0,f0])
          clea_mat

          # Create point layer
          suppressWarnings({
            cst_sf <- suppressMessages(
              st_as_sf(clea_mat,coords = c("long", "lat"),crs=st_crs(map), na.fail=FALSE) %>% st_crop(st_bbox(map))
            )
          })
...
# Close error catch
},error=function(e){message(paste0("ERROR!!! k0=",k0," " ,sub_dirz$iso3[k0]," ",sub_dirz$yr[k0]));message(e)})
