...
sub_dirz[,which(adm1=="FL"&yr==1916)]
k0 <- 186; sub_dirz[k0,]
  ...
  # for(a0 in length(admz)){#print(paste0("a0=",a0))
    ...
    if(nrow(admz_dir_)>0){
      ...
      for(a0_ in 1:nrow(admz_dir_)){#print(paste0("a0_=",a0_))
        ...
        # File name
        fnn <- paste0("ocgub_",sub_dirz$iso3[k0],"_",sub_dirz$adm1[k0],"_",sub_dirz$yr[k0],"_",admz[a0],admz_dir_$yr[a0_],"_",admz_dir_$adm[a0_],".RDS")
          ...
          if(nrow(map)>0){
            ...
            if(nrow(map)>0&nrow(oc_mat[!is.na(longitude),])>0){
              ...
              if(!skip_existing|(skip_existing&!fnn%in%dir(paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_simp/")))){
                ...
                oc_chars_0 <- point2poly_simp(polyz=map,pointz=cst_sf,varz=char_varz[!char_varz%in%names(map)],funz=function(x){paste0(unique(x),collapse="|")},na_val=NA)
