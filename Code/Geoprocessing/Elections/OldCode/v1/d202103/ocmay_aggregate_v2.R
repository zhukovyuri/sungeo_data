...
# nohup R -e 'source("/data/Dropbox/SUNGEO/Code/ElectionData/Geoprocessing/ocmay_aggregate_v2.R")' &
# nohup R -e 'source("~/Dropbox/SUNGEO/Code/ElectionData/Geoprocessing/ocmay_aggregate_v2.R")' &
# tail -f nohup.out
# R
...
detachAllPackages()
list.of.packages <- c("sf","raster","tidyverse","data.table","countrycode","parallel","maptools","stringi","plyr")
...
oc_supp <- readRDS("../../SUNGEO_Flu/Data/RawData/USA/OurCampaigns/may_supp.RDS") %>% as.data.table() 
sub_dirz <- dir("Elections/OurCampaigns/Preprocessed/Mayoral") %>% str_split("_|.RDS",simplify=T) %>% .[,2:3] %>% as.data.frame(stringsAsFactors=T) %>% data.table::setnames(c("RaceID","yr")) %>% dplyr::mutate(RaceID = RaceID %>% as.character(), yr = yr %>% as.character() %>% as.numeric(), f0 = list.files("Elections/OurCampaigns/Preprocessed/Mayoral",full.names=T)) %>% as.data.table() %>% .[,iso3 := "USA"] %>% .[,.(iso3,RaceID,yr,f0)] %>% arrange(iso3,RaceID,yr) %>% as.data.table() %>% .[, state := match(RaceID,oc$RaceID) %>% oc[.,State]] %>% .[, city := match(RaceID,oc$RaceID) %>% oc[.,gsub(unique(State) %>% paste0(", ",.,"| \\(",.,"\\)| \\[",.,"\\]| ",.,collapse="|"),"",City)]] %>% .[is.na(state), state := match(RaceID,oc_supp$RaceID) %>% oc_supp[.,State]] %>% .[is.na(city), city := match(RaceID,oc_supp$RaceID) %>% oc_supp[.,City]]
rm(oc,oc_supp)
...
tryCatch({
  ...
  # for(a0 in length(admz)){#print(paste0("a0=",a0))
    ...
    admz_dir_ <- admz_dir[iso3 %in% sub_dirz[yr%in%yrz[k0],iso3],]
    ...
    if(nrow(admz_dir_)>0){
      ...
      for(a0_ in 1:nrow(admz_dir_)){#print(paste0("a0_=",a0_))
        ...
        # File name
        fnn <- paste0("ocmay_",sub_dirz[yr%in%yrz[k0],paste0(iso3,"_",yr)],"_",admz[a0],admz_dir_$yr[a0_],"_",admz_dir_$adm[a0_],".RDS") %>% .[1]
        ...
        if(!skip_existing|(skip_existing
          ...
          if(nrow(map)>0){
            ...
            oc_mat <- lapply(sub_dirz[yr%in%yrz[k0],f0],readRDS)
            class(oc_mat)
            if(length(oc_mat)==1){oc_mat <- oc_mat[[1]]}
            if(length(oc_mat)>1&"list"%in%class(oc_mat)){
              oc_mat_ <- oc_mat[[1]]
              oc_mat_$osm_id <- as.character(oc_mat_$osm_id)
              j0_ <- 2
              for(j0_ in 2:length(oc_mat)){#print(j0_)
                oc_mat_2 <- oc_mat[[j0_]]
                oc_mat_2$osm_id <- as.character(oc_mat_2$osm_id)
                oc_mat_ <- plyr::rbind.fill(oc_mat_,oc_mat_2)
              }
              oc_mat <- oc_mat_; rm(oc_mat_)
            }
            oc_mat <- oc_mat %>% as.data.table() %>% .[vv1>1|noncontested==1,] %>% .[,city := gsub(unique(state) %>% paste0(", ",.,"| \\(",.,"\\)| \\[",.,"\\]| ",.,collapse="|"),"",city) %>% stringr::str_trim()]
