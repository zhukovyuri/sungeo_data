...
# nohup R -e 'source("/data/Dropbox/SUNGEO/Code/ElectionData/Geoprocessing/ocgub_aggregate.R")' &
# nohup R -e 'source("~/Dropbox/SUNGEO/Code/ElectionData/Geoprocessing/ocgub_aggregate.R")' &
# tail -f nohup.out
# R
...
sub_dirz <- dir("Elections/OurCampaigns/Preprocessed/Gubernatorial") %>% str_split("_|.RDS",simplify=T) %>% .[,2:3] %>% as.data.frame(stringsAsFactors=T) %>% data.table::setnames(c("adm1","yr")) %>% dplyr::mutate(adm1 = adm1 %>% as.character(), yr = yr %>% as.character() %>% as.numeric(), f0 = list.files("Elections/OurCampaigns/Preprocessed/Gubernatorial",full.names=T)) %>% as.data.table() %>% .[,iso3 := "USA"] %>% .[,.(iso3,adm1,yr,f0)] %>% arrange(iso3,adm1,yr) %>% as.data.table()
which(sub_dirz$iso3=="USA"&sub_dirz$yr==1914)

yrz <- sub_dirz[,unique(yr)] %>% sort()
...
# Forking
# 1:length(yrz)
cntz_list <- mclapply(which(yrz %in% c(1898:1920,2000:2020)),function(k0){
...
tryCatch({
  ...
  print(paste0(yrz[k0],", ",k0,"/",length(yrz)," (",round(k0/length(yrz),3)*100,"%)"))  
  ...
  # for(a0 in length(admz)){#print(paste0("a0=",a0))
    ...
    # print(paste0(sub_dirz$iso3[k0]," ",sub_dirz$adm1[k0]," ",sub_dirz$yr[k0]," ",admz[a0],", ",a0,"/",length(admz)))
    ...
    admz_dir_ <- admz_dir_[yr %in% c(1910,1916,2014,2010,2017),]
    # admz_dir_ <- admz_dir_[8]
    ...
    if(nrow(admz_dir_)>0){
      ...
      for(a0_ in 1:nrow(admz_dir_)){#print(paste0("a0_=",a0_))
        ...
        # File name
        fnn <- paste0("ocgub_",sub_dirz[yr%in%yrz[k0],paste0(iso3,"_",yr)],"_",admz[a0],admz_dir_$yr[a0_],"_",admz_dir_$adm[a0_],".RDS") %>% .[1]

        # Execute only if one of the files doesn't exist
        if(!skip_existing|(skip_existing
          &(!fnn%in%dir(paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_simp/"))
          |!fnn%in%dir(paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_tess/"))
          # |!fnn%in%dir(paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_krig/"))
          ))){
          ...
          # Specify variables
          char_varz <- c("sid","election_id","sname","state","year","yrmo","noncontested","sincumb_ptyname","sincumb_pty","sincumb_can","incumb_ptyname","incumb_pty","incumb_can","win1_ptyname","win1_pty","win1_can","win2_ptyname","win2_pty","win2_can")
          mean_varz <- c("to1","cvs1_margin","pvs1_margin","pvs1_margin_incumb","pvs1_margin_sincumb","vs1_margin","to2","cvs2_margin","pvs2_margin","vs2_margin","win_pvs1","win_cvs1","pvs1_sincumb","cvs1_incumb","pvs1_incumb","contest_p1_sincumb","contest_p1","contest_c1","comptop2_c1","comptop2_p1","comptop1_c1","comptop1_p1")
          ...
          if(nrow(map)>0){
            ...
            # Load oc
            oc_mat <- lapply(sub_dirz[yr%in%yrz[k0],f0],readRDS) %>% data.table::rbindlist(fill=T)
            ...
            if(nrow(map)>0&nrow(oc_mat[!is.na(longitude),])>0){
              ...
              # Skip if file exists
              if(!skip_existing|(skip_existing&!fnn%in%dir(paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_simp/")))){

                # print(paste0(fnn %>% gsub("ocgub_|.RDS","",.)," -- started simple overlay"))
                ...
                oc_out <- map %>% 
                  ...
                  data.table::setnames(old=c("election_id","sid","sname","state","year","yrmo"),new=c("OC_ID","OC_SID","OC_SID_N","OC_ADM1_N","OC_YEAR","OC_YRMO"),skip_absent=TRUE) %>% 
                ...
                # Save 
                saveRDS(oc_out,file=paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_simp/",fnn))

                # # Visualize
                # tryCatch({
                #     png(paste0("Elections/OurCampaigns/Maps/Gubernatorial/",admz[a0],"_simp/",fnn %>% gsub(".RDS","_000.png",.)),width=4*1.618,height=4,units="in",res=100)
                #     par(mar=c(0,0,0,0))
                ...
                #       png(paste0("Elections/OurCampaigns/Maps/Gubernatorial/",admz[a0],"_simp/",fnn %>% gsub(".RDS",paste0("_",plot_var,".png"),.)),width=4*1.618,height=4,units="in",res=100)
                ...
                print(paste0(fnn %>% gsub("ocgub_|.RDS","",.)," -- finished simple overlay"))
              ...
              # # Skip if file exists
              # if(!skip_existing|(skip_existing&!fnn%in%dir(paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_tess/")))){

              # # print(paste0(fnn %>% gsub("ocgub_|.RDS","",.)," -- started tess"))
              ...
              #     v_pop <- c(4,3)[c(yrz[k0]>=2000,yrz[k0]<2000)]
              #     fnam1 <- c("/gpw-v4-population-count-rev11_20","/gl_gpwv3_pcount_")[c(yrz[k0]>=2000,yrz[k0]<2000)]
              #     fnam2 <- c("_2pt5_min_asc.zip","_ascii_25.zip")[c(yrz[k0]>=2000,yrz[k0]<2000)]
              #     r_years <- cbind(1990:2024,rep(seq(1990,2020,by=5),each=5))
              #     con_pop <- unzip(zipfile = paste0("Population/GPW_v",v_pop,fnam1,substr(r_years[match(yrz[k0],r_years[,1]),2],3,4),fnam2), exdir = temp)
              ...
              #     data.table::setnames(old=c(paste0(rep(c("election_id","sid","sname","state"),each=2),c("_aw","_pw")),"year","yrmo","noncontested_aw"),new=c(paste0(rep(c("OC_ID","OC_SID","OC_SID_N","OC_ADM1_N"),each=2),c("_AW","_PW")),"OC_YEAR","OC_YRMO","OC_NONCONTESTED"),skip_absent=TRUE) %>% 
              #     dplyr::mutate(POLYGONS = fnn %>% str_split("_",simplify=T) %>% .[,4] ) %>%
              #     dplyr::select(POLYGONS,OC_YRMO,OC_YEAR,everything()) %>% 
              #     # dplyr::select(-one_of("year_pw","yrmo_pw","noncontested_pw")) %>% 
              #     data.table::setnames(old=names(.)[names(.)!="geometry"],new=names(.)[names(.)!="geometry"] %>% toupper())

              #   # Save 
              #   saveRDS(oc_out,file=paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_tess/",fnn))

              #   # # Visualize
              #   # tryCatch({
              #   #     png(paste0("Elections/OurCampaigns/Maps/Gubernatorial/",admz[a0],"_tess/",fnn %>% gsub(".RDS","_000_vor.png",.)),width=4*1.618,height=4,units="in",res=100)
              #   #     par(mar=c(0,0,0,0))
              ...
              #   #       tryCatch({
              #   #       oc_sf <- oc_out %>% st_as_sf()
              #   #       png(paste0("Elections/OurCampaigns/Maps/Gubernatorial/",admz[a0],"_tess/",fnn %>% gsub(".RDS",paste0("_",plot_var %>% toupper(),"_AW.png"),.)),width=4*1.618,height=4,units="in",res=100)
              ...
              #   print(paste0(fnn %>% gsub("ocgub_|.RDS","",.)," -- finished tess"))
              ...
              # # Skip if file exists
              # if(!skip_existing|(skip_existing&!fnn%in%dir(paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_krig/")))){
              ...
              #   # Save 
              #   saveRDS(oc_out,file=paste0("Elections/OurCampaigns/Processed/Gubernatorial/",admz[a0],"_krig/",fnn))
              ...
              #         tryCatch({
              #           png(paste0("Elections/OurCampaigns/Maps/Gubernatorial/",admz[a0],"_krig/",fnn %>% gsub(".RDS",paste0("_",plot_var,".png"),.)),width=4*1.618,height=4,units="in",res=100)
              ...
              # print(paste0(fnn %>% gsub("ocgub_|.RDS","",.)," -- finished kriging"))
              ...
              # # Close error catch
              # },error=function(e){message(paste0("ERROR!!! ",fnn %>% gsub("ocgub_|.RDS","",.)," krig"));message(e)})
...
# Close error catch
},error=function(e){message(paste0("ERROR!!! k0=",k0," " ,yrz[k0]));message(e)})
...
# Close mclapply
},mc.cores = min(length(yrz),ncores))
# gc()
