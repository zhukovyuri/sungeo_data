...
# nohup R -e 'source("/data/Dropbox/SUNGEO/Code/ElectionData/Geoprocessing/prio_aggregate.R")' &
# nohup R -e 'source("~/Dropbox/SUNGEO/Code/ElectionData/Geoprocessing/prio_aggregate.R")' &
# tail -f nohup.out
# R
...
prio <- map0 %>% st_as_sf() %>% setnames(old=c("gid","xcoord","ycoord","col","row"),new=c("PRIO_GID","PRIO_X","PRIO_Y","PRIO_COL","PRIO_ROW"))
st_crs(prio) <- 4326
prio
rm(map0)
...
ne_cntz <- sort(unique(map0_ne$ISO3))
k0 <- ne_cntz[11]
prio_list <- mclapply(ne_cntz,function(k0){
  t1 <- Sys.time()
  print(paste0(k0," starting"))
  map0_sub <- map0_ne[map0_ne$ISO3%in%k0,] 
  o0 <- suppressMessages(
    prio %>% st_centroid() %>% st_within(map0_sub %>% st_buffer(dist=.5)) 
    )
  prio_crop <- prio[o0 %>% lengths() > 0,]
  prio_crop <- suppressMessages(
    prio_crop[prio_crop %>% st_intersects(map0_sub) %>% lengths() > 0,] %>% mutate(ISO3 = k0) %>% select(ISO3,everything())
    )
  save(prio_crop,file=paste0(temp,"/PRIO_",k0,".RData"))
  # png(paste0("Admin/PRIOGRID/Maps/000_",k0,".png"),width=10,height=10,units="in",res=150)
  # plot(prio_crop$geometry)
...
for(y0 in 1:length(yrz)){
  ...
  # Exceptions
  clea_cntz <- clea_cntz[!clea_cntz%in%skip_prio]
  ...
  save_list <- mclapply(seq_along(clea_cntz),function(k0){
    ...
    b <- as(extent(st_bbox(map0_ne[map0_ne$ISO3%in%clea_cntz[k0],] )[c(1,3,2,4)] %>% as.numeric()), 'SpatialPolygons')
    crs(b) <- crs(r)
    b <- st_as_sf(b) 
    r_crop <- crop(r, b)
  ...
  cntz_list <- mclapply(seq_along(clea_cntz),function(k0){
    ...
    if(!skip_existing|(skip_existing&!paste0("PRIO_CLEA_",clea_cntz[k0],"_",yrz[y0],".RData")%in%dir("Elections/CLEA/Processed/PRIO_PointInPoly/"))){

      # Load PG polygons
      load(paste0(temp,"/PRIO_",clea_cntz[k0],".RData"))

      # Load country borders
      map0_ <- map0_ne[map0_ne$ISO3%in%clea_cntz[k0],] 
      ...
      clea_chars_0 <- pinp_fun(polyz=prio_crop,pointz=cst_sf,varz=char_varz,funz=function(x){paste0(unique(x),collapse="|")},na_val=NA)
      clea_means_0 <- pinp_fun(polyz=prio_crop,pointz=cst_sf,varz=mean_varz,funz=function(x){mean(x,na.rm=T)},na_val=NA)
      clea_sums_0 <- pinp_fun(polyz=prio_crop,pointz=cst_sf,varz=sum_varz,funz=function(x){sum(x,na.rm=T)},na_val=NA)
      ...
      # Save 
      save(clea_prio,file=paste0("Elections/CLEA/Processed/PRIO_PointInPoly/PRIO_CLEA_",clea_cntz[k0],"_",yrz[y0],".RData"))

      # Visualize
      tryCatch({
        png(paste0("Elections/CLEA/Maps/PRIO_PointInPoly/",clea_cntz[k0],"_",yrz[y0],"_000.png"),width=4,height=4,units="in",res=150)
          plot(clea_prio$geometry,lwd=.25)
          points(as(cst_sf,"Spatial"))
        dev.off()
        for(plot_var in toupper(mean_varz)){
          if(clea_prio[plot_var] %>% na.omit() %>% nrow() > 0){
            clea_sf <- clea_prio %>% st_as_sf()
            png(paste0("Elections/CLEA/Maps/PRIO_PointInPoly/",clea_cntz[k0],"_",yrz[y0],"_",plot_var,"_prio.png"),width=4,height=4,units="in",res=150)
    ...
    # Clean workspace
    rm(list=ls()[grep("clea_prio|_char|_means|_sums|cst_sf",ls())])
    gc(reset=T)
    ...
    if(!skip_existing|(skip_existing&!paste0("PRIO_CLEA_",clea_cntz[k0],"_",yrz[y0],".RData")%in%dir("Elections/CLEA/Processed/PRIO_Voronoi/"))){
      ...
      # Rename variables
      geo_vor <- clea_prio_list[["tess"]] %>%
        setnames(old=c("cst","cst_n","ctr","ctr_n","yr","yrmo"),new=c("CLEA_CST","CLEA_CST_N","CLEA_CTR","CLEA_CTR_N","YEAR","YRMO"),skip_absent=TRUE) %>% 
        select(ISO3,YRMO,YEAR,everything()) %>% 
        setnames(old=names(.)[names(.)!="geometry"],new=names(.)[names(.)!="geometry"] %>% toupper()) %>% st_as_sf()
      clea_prio <- clea_prio_list[["result"]] %>%
      ...
      # Save 
      save(geo_vor,clea_prio,file=paste0("Elections/CLEA/Processed/PRIO_Voronoi/PRIO_CLEA_",clea_cntz[k0],"_",yrz[y0],".RData"))

      # Visualize
      tryCatch({
        png(paste0("Elections/CLEA/Maps/PRIO_Voronoi/",clea_cntz[k0],"_",yrz[y0],"_000_voronoi.png"),width=4,height=4,units="in",res=150)
        plot(geo_vor$geometry,lwd=.25)
        dev.off()
        for(plot_var in toupper(mean_varz)){#print(plot_var)
          if(geo_vor[plot_var] %>% na.omit() %>% nrow() > 0){
            png(paste0("Elections/CLEA/Maps/PRIO_Voronoi/",clea_cntz[k0],"_",yrz[y0],"_",plot_var,"_voronoi.png"),width=4,height=4,units="in",res=150)
            plot(geo_vor[plot_var],lwd=.25)
            dev.off()
            png(paste0("Elections/CLEA/Maps/PRIO_Voronoi/",clea_cntz[k0],"_",yrz[y0],"_",plot_var,"_pw_prio.png"),width=4,height=4,units="in",res=150)
            plot(clea_prio[paste0(plot_var,"_PW")],lwd=.25)
            dev.off()
            png(paste0("Elections/CLEA/Maps/PRIO_Voronoi/",clea_cntz[k0],"_",yrz[y0],"_",plot_var,"_aw_prio.png"),width=4,height=4,units="in",res=150)
            plot(clea_prio[paste0(plot_var,"_AW")],lwd=.25)
            dev.off()
          }
        }
      },error=function(e){})
    ...
    if(!skip_existing|(skip_existing&!paste0("PRIO_CLEA_",clea_cntz[k0],"_",yrz[y0],".RData")%in%dir("Elections/CLEA/Processed/PRIO_Kriging/"))){
      ...
      # Ordinary kriging (Returning all NA's...???)
      clea_prio <- krig_fun(
        pointz=cst_sf,
        polyz=prio_crop,
        varz=mean_varz,
      ...
      # Rename variables
      clea_prio[,c("yr","yrmo"):=list(cst_sf$yr[1],cst_sf$yrmo[1])]
      clea_prio <- clea_prio %>%
        setnames(old=c("cst","cst_n","ctr","ctr_n","yr","yrmo"),new=c("CLEA_CST","CLEA_CST_N","CLEA_CTR","CLEA_CTR_N","YEAR","YRMO"),skip_absent=TRUE) %>% 
        select(ISO3,YRMO,YEAR,everything()) %>% 
        setnames(old=names(.)[names(.)!="geometry"],new=names(.)[names(.)!="geometry"] %>% toupper()) %>% st_as_sf()
      ...
      # Save 
      save(clea_prio,file=paste0("Elections/CLEA/Processed/PRIO_Kriging/PRIO_CLEA_",clea_cntz[k0],"_",yrz[y0],".RData"))

      # Visualize
      tryCatch({
        for(plot_var in toupper(mean_varz)){#print(plot_var)
          if(clea_prio[paste0(plot_var,"_KR")] %>% na.omit() %>% nrow() > 0){
            png(paste0("Elections/CLEA/Maps/PRIO_Kriging/",clea_cntz[k0],"_",yrz[y0],"_",plot_var,"_prio.png"),width=4,height=4,units="in",res=150)
            plot(clea_prio[paste0(plot_var,"_KR")],lwd=.25)
            dev.off()
          }
        }
      },error=function(e){})
