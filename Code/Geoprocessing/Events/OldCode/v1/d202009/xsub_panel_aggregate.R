...
adz <- data.frame(
  ...
  prec = c("adm0|adm1|adm2|settlement","adm1|adm2|settlement","adm2|settlement","adm1|adm2|settlement"),
...
cntz_list <- mclapply(1:nrow(sub_dirz),function(k0){
  ...
  load(sub_dirz[k0,f0])
  xsub_data <- indata %>% filter((!is.na(LONG))&(!is.na(LAT))) %>% dplyr::mutate_if(is.factor, list(~ as.character(.))) %>% merge(ticker,by="DATE",all.x=T,all.y=F) %>% as.data.table() ; rm(indata)
    ...
    if(nrow(admz_dir_)>0){
      ...
      for(a0_ in 1:nrow(admz_dir_)){
        ...
        tunit_list <- lapply(1:nrow(timz),function(t0){
          ...
          # File name
          fnn <- paste0("xSub_",sub_dirz$source[k0],"_",sub_dirz$iso3[k0],"_",admz[a0],admz_dir_$yr[a0_],"_",admz_dir_$adm[a0_],"_",timz[t0,tunit] %>% toupper(),".RDS")

          # Execute only if one of the files doesn't exist
          if(!skip_existing|(skip_existing&(!fnn%in%dir(paste0("EventData/xSub/Processed/xSub_Panel/",admz[a0],"_simp/"))))){
            ...
            if(nrow(map)>0&nrow(xsub_data[grepl(adz[sunit%in%admz_dir_[a0_,adm],prec],GEOPRECISION)&grepl(timz[t0,prec],TIMEPRECISION),])>0){

              # Create sf layer
              xsub_sf <- st_as_sf(xsub_data[grepl(adz[sunit%in%admz_dir_[a0_,adm],prec],GEOPRECISION)&grepl(timz[t0,prec],TIMEPRECISION),],coords = c("LONG", "LAT"),crs=st_crs(map)) %>% dplyr::mutate(TUNIT = timz[t0,tvar] %>% get())
              ...
              all_tint <- ticker[,timz[t0,tvar] %>% get()] %>% unique() %>% .[.>=min(xsub_tint) & .<=max(xsub_tint)]
              # xsub_tout <- all_tint[!all_tint%in%xsub_tint]

              # Event counts each time interval
              # t0_ <- 2019
              xsub_out <- lapply(all_tint,function(t0_){
                t_mat_0 <- point2poly_simp(polyz=map,
                    pointz=xsub_sf[xsub_sf$TUNIT %in% t0_,],
              ...
              # Save 
              saveRDS(xsub_out,file=paste0("EventData/xSub/Processed/xSub_Panel/",admz[a0],"_simp/",fnn))
