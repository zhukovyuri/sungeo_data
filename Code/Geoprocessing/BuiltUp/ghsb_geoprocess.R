...
# nohup R -e 'source("/data/Dropbox/SUNGEO/Code/BuiltUp/Geoprocessing/ghsb_geoprocess.R")'  > sungeo_ghsb.log 2>&1 &  
# nohup R -e 'source("/media/zhukov/dropbox2022/Dropbox/SUNGEO/Code/BuiltUp/Geoprocessing/ghsb_geoprocess.R")'  > sungeo_ghsb.log 2>&1 &  
# tail -f sungeo_ghsb.log
# R
    ...
    if(!skip_existing|(skip_existing&!file.exists(sub_dirz[k0,fnn]))){
			...
			if(sum(sf::st_is_empty(map))==0&nrow(map)>0){
				...
				bu.mat <- parallel::mclapply(1:nrow(ticker),function(t){
					...
					# plot(f_tif_m0)
					# dev.off()
					# rnkz <- map_z[,rank(busurf_sum)/max(rank(busurf_sum))]
