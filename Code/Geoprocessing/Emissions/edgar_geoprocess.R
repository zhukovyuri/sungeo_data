...
# # QC
# qc <- readRDS("Data/QC/qc_20250112.RDS")
# sub_dirz <- sub_dirz %>% .[,c("error_any","error_unbalanced","error_missingx","error_zerorows"):=fnn%>%match(qc[,fnn])%>%qc[.,.(error_any,error_unbalanced,error_missingx,error_zerorows)]]
	...
	tryCatch({
	    ...
	    if(!skip_existing|(skip_existing&!file.exists(sub_dirz[k0,fnn]))){
			...
			if(sum(sf::st_is_empty(map))==0&nrow(map)>0){
				...
				t <- 1
				m0 <- 1
				co2.mat <- parallel::mclapply(1:nrow(ticker),function(t){
					...
					# 	edgar_nc <- terra::project(edgar_nc,terra::crs(map))
					# }
					...
					map_z <- map_z %>% data.table::as.data.table() %>% .[,SG_POLYID:=poly_idz] %>% .[,YEAR:=ticker[t,YEAR]] %>% .[,emissions_sum:=map_z_sum$emissions] %>% dplyr::select("SG_POLYID","YEAR",dplyr::everything())
				...
				# co2.mat[,length(unique(POLYGON_ID))]
