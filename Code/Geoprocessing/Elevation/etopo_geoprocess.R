...
# nohup R -e 'source("/data/Dropbox/SUNGEO/Code/Elevation/Geoprocessing/etopo_geoprocess.R")' &
# tail -f nohup.out
# R
...
# Log file
log_file <- "../r_log_errors_etopo.txt"
...
# Load raster
etopo <- list.files("Elevation/ETOPO1/Raw",pattern="tif$", full.names=T) %>% raster::raster()
# etopo <- list.files("Elevation/ETOPO1/Raw",pattern="zip$", full.names=T) %>% unzip(exdir=tempd) %>% raster::raster()
raster::crs(etopo) <- raster::crs("+init=epsg:4326")
...
# parallel::clusterExport(NULL,c("skip_existing","sub_dirz","admz","ncores","mem_all","mem0","log_file","etopo"),envir = environment())
...
# print(paste0("starting ",sub_dirz[k0,fnn] %>% stringr::str_split("/",simplify=T) %>% .[5] %>% gsub("ETOPO1_|.RDS","",.)))
  ...
  tryCatch({
	...
	if(nrow(map)>0){
		...
		bbx <- sf::st_bbox(map)
		etopo_ <- etopo %>% raster::crop(bbx[c("xmin","xmax","ymin","ymax")] %>% raster::extent())
		etopo_file <- etopo_ %>% capture.output() %>% grep("^source",.,value=TRUE) %>% stringr::str_split(":",simplify=TRUE) %>% .[2] %>% stringr::str_trim()

		# Calculate slope
		slope_ <- etopo_ %>% raster::terrain(unit="degrees")
		slope_file <- slope_ %>% capture.output() %>% grep("^source",.,value=TRUE) %>% stringr::str_split(":",simplify=TRUE) %>% .[2] %>% stringr::str_trim()
		# raster::plot(slope_)
		
		# Extract raster stats
		map00.0 <- map %>% as.data.table() %>% .[,c("ETOPO1_VERSION") := .(names(etopo_))] %>% .[,ETOPO_ELEV_MEAN := raster::extract(etopo_,map,fun=mean,factors=T,buffer=1000, small=T,na.rm=T)]  %>% .[,ETOPO_ELEV_SD := raster::extract(etopo_,map,fun=sd,factors=T,buffer=1000, small=T,na.rm=T)] %>% .[,ETOPO_ELEV_MIN := raster::extract(etopo_,map,fun=min,factors=T,buffer=1000, small=T,na.rm=T)] %>% .[,ETOPO_ELEV_MAX := raster::extract(etopo_,map,fun=max,factors=T,buffer=1000, small=T,na.rm=T)] %>% .[,ETOPO_SLOPE_MEAN := raster::extract(slope_,map,fun=mean,factors=T,buffer=1000, small=T,na.rm=T)]  %>% .[,ETOPO_SLOPE_SD := raster::extract(slope_,map,fun=sd,factors=T,buffer=1000, small=T,na.rm=T)] %>% .[,ETOPO_SLOPE_MIN := raster::extract(slope_,map,fun=min,factors=T,buffer=1000, small=T,na.rm=T)] %>% .[,ETOPO_SLOPE_MAX := raster::extract(slope_,map,fun=max,factors=T,buffer=1000, small=T,na.rm=T)] #%>% .[,geometry := NULL]

		# plot(map00.0 %>% sf::st_as_sf() %>% .["ETOPO_SLOPE_MAX"]); 
		# dev.off()

		etopo_mat <- map00.0 %>% .[,POLYGON_ID := paste0(sub_dirz[k0,paste0(iso3,"_",geoset,adm_yr,"_",adm)],"_",get(idvar))] %>% .[,.SD,.SDcols=c("POLYGON_ID",idvar,grep("^ETOPO",names(.),value=TRUE))]

		# # Delete temporary files
		rm(etopo_,slope_)
		if(etopo_file!="memory"){file.remove(c(etopo_file,gsub("grd$","gri",c(etopo_file))))}
		if(slope_file!="memory"){file.remove(c(slope_file,gsub("grd$","gri",c(slope_file))))}
		
  		# Save
		saveRDS(etopo_mat,file=sub_dirz[k0,fnn])

		# Progress
		print(paste0(sub_dirz[k0,fnn] %>% stringr::str_split("/",simplify=T) %>% .[5] %>% gsub("ETOPO1_|.RDS","",.)," -- finished  :)"))
        t2 <- Sys.time()
        print(t2-t1)
