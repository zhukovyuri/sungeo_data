...
# sparkR --master yarn --queue zhukov_q1 --num-executors 4 --executor-memory 1g --executor-cores 4
...
if(grepl("arc-ts|^node",Sys.info()[["nodename"]])){setwd("SUNGEO/Data/")}
...
parallel::clusterExport(NULL,c("skip_existing","sub_dirz","admz","ncores","mem_all","mem0","log_file","long.alt","long","lat","weather.times","rain.times","xdata","xdata2"),envir = environment())
	    ...
	    if(!skip_existing|(skip_existing&!sub_dirz[k0,exists])){
			...
			if(sum(sf::st_is_empty(map))==0&nrow(map)>0){
				...
				weather.mat <- lapply(1:nrow(weather.times),function(t){
					...
					# # Print at each year
					# sub_dirz[k0,adm]
					# if(grepl("01$",as.character(rain.times[t,"YRMO"]))){print(sub_dirz[k0,paste(geoset,adm,iso3,adm_yr,as.character(rain.times[t,"YRMO"]))])}
